...
function unitBucket(key) {
  if (key.includes("_tower")) return "towers";
  if (key.startsWith("npc_dota_creep_") || key.includes("_siege")) {
    return "creeps";
  }
  if (key.startsWith("npc_dota_neutral_")) return "neutrals";
...
function processDamageTaken(damageTaken) {
  ...
  Object.entries(damageTaken).forEach(([source, damage]) => {
    const bucket = unitBucket(source);
...
- secondsDead: Total time each player spent dead, after buybacks
- deathLog: The focus player's deaths. goldLost appears only where the death cost gold, which a Turbo death never does
...
DAMAGE ANALYSIS
- Use damageTaken and the killers in deathLog to identify survivability issues and positioning problems; cite the times of deaths to towers, creeps or neutrals.
