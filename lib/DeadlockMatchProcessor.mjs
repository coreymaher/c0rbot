 ...
 * @param {number} team - Team that owned/lost the objective (0 or 1)
 ...
 */
function getObjectiveName(team, objectiveId) {
  return OBJECTIVE_NAMES[objectiveId] || `Unknown Objective (id ${objectiveId})`;
}
    ...
    for (const obj of matchData.objectives) {
      if (obj.destroyed_time_s > 0) {
        events.push({
          ...
          objective_name: getObjectiveName(obj.team, obj.team_objective_id),
...
export function generateCompactMatch(matchData, focusPlayerId, itemsData) {
  ...
  const focusPlayer = matchData.players.find((p) => p.account_id === focusPlayerId);
  if (focusPlayer) {
 ...
 */
export function generateAnalysisPrompt(compactMatch, playerId, playerName) {
