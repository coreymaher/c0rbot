...
module.exports.pokemongoUpdates = (event, context, callback) => {
  ...
  Promise.all([dbPromise, requestPromise])
    .then((values) => {
      ...
      const urlLink =
        link.length > 0
          ? `http://pokemongo.nianticlabs.com${link.attr("href")}`
          : url;
...
module.exports.twitchStreams = (event, context, callback) => {
  ...
  const promises = channels.map((channel) => {
    ...
    Promise.all([dbPromise, requestPromise]).then((values) => {
      ...
      if (
        response.stream &&
        (!("Item" in db) || db.Item.feed_data != response.stream._id)
      ) {
        const embed = {
...
module.exports.arkChangelog = (event, context, callback) => {
  ...
  function parseVersion(content) {
    ...
    return matches.length > 1
      ? { version: matches[1], url: link.attr("href") }
      : null;
  }
  ...
  Promise.all([dbPromise, requestPromise])
    .then((values) => {
      ...
      if (
        version &&
        (!("Item" in db) || db.Item.feed_data != version.version)
      ) {
...
module.exports.fortniteChangelog = (event, context, callback) => {
  ...
  Promise.all([dbPromise, requestPromise])
    .then((values) => {
      ...
      const url =
        "externalLink" in changePost
          ? changePost.externalLink
          : changePost.urlPattern;
  ...
  Promise.all([dbPromise, requestPromise])
    .then((values) => {
      ...
      return new Promise((resolve, reject) => {
        if (!("Item" in db) || db.Item.feed_data != id) {
          const updates = [
            ...Array(Math.min(3, latest.PatchNotes.length)).keys(),
          ].map((i) => {
