    ...
    return Promise.all(promises)
        .then((matches) => {
            matches.forEach((match) => {
                ...
                const matchTimestamp = Math.floor(new Date(match.data.attributes.createdAt).getTime() / 1000);
                ...
                match.included.forEach((obj) => {
                    Object.keys(data.users).forEach((userID) => {
                        ...
                        if (obj.type === 'participant' && obj.attributes.stats.playerId == userID) {
                            if (match.data.id != user.last_match_id && matchTimestamp > user.last_match_timestamp) {
                                if (obj.attributes.stats.winPlace <= 10 || obj.attributes.stats.winPlace == 100) {
                                    messages.push(buildEmbed(obj.attributes.stats.winPlace, obj, match.data));
                                }
                            }
                        }
                    });
...
function buildEmbed(place, player, match) {
    ...
    if (place == 1) {
        ...
        description = `${name} just finished ${ordinal(place)} in a ${type} PUBG match`;
    }
...
function updateDB(data) {
    ...
    Object.keys(data.users).forEach((id) => {
        ...
        const params = {
            ...
            UpdateExpression: 'SET last_match_id = :last_match_id, last_match_timestamp = :last_match_timestamp, account_name = :account_name, updated_at = :updated_at',
    ...
    return Promise.all(updates).then(() => { return Promise.resolve(data); });
}
