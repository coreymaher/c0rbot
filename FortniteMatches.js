    ...
    Object.keys(data.users).map((name) => {
        ...
        if (user.stats) {
            [ 'solo', 'duo', 'squad' ].forEach((type) => {
                if ((type in user.stats) && user.newStats[type].matches > user.stats[type].matches) {
                    user.updated = true;
                    ...
                    Object.keys(user.stats[type].top).forEach((result) => {
                        ...
                        if (user.newStats[type].top[result] > user.stats[type].top[result]) {
                            messages.push(buildEmbed({
                                name: user.name,
