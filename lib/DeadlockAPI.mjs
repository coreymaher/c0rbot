...
class DeadlockAPI {
   ...
   * @private
   */
  async #fetchPopularItemsFromAPI(heroId, minBadge) {
    const controller = new AbortController();
    ...
    const url = `${API_PREFIX}analytics/item-stats?hero_ids=${heroId}&min_average_badge=${minBadge}&min_unix_timestamp=${sevenDaysAgo}`;
   ...
   */
  async getPopularItems(heroId, minBadge, { skipCache = false } = {}) {
    const cacheKey = `popular-items:hero-${heroId}:badge-${minBadge}:v1`;
    ...
    // Fetch from API
    try {
      const data = await this.#fetchPopularItemsFromAPI(heroId, minBadge);
      if (!data) return null;
