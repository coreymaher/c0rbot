    ...
    metaItems = popularItemsData
      ...
      .map((popularItem) => {
        ...
        return {
          ...
          pick_rate: popularItem.pick_rate
            ? Math.round(popularItem.pick_rate * 100)
            : null,
          win_rate: popularItem.win_rate
            ? Math.round(popularItem.win_rate * 100)
            : null,
        };
      })
...
- all_items_reference is a map where keys are class_name identifiers and values contain: name (display name), description, slot_type, is_active, cost, and properties.
...
- Analyze itemization strategy by examining item descriptions, properties, slot types (weapon/vitality/spirit), and costs from all_items_reference.
...
META ITEMS CONTEXT
- The focus player may have a meta_items field showing popular items for their hero at higher skill levels (sorted by win rate).
- Each meta item includes: item (class_name identifier), pick_rate (%), and win_rate (%).
- Look up the item field in all_items_reference to get full item details (name, description, properties, cost).
- Use meta_items to evaluate if the player's itemization aligned with proven high-win-rate builds.
- Consider whether deviations from meta were situationally appropriate (countering specific enemies, filling team gaps).
- Meta builds are guidelines, not rules - adaptive itemization for match context is valid and often necessary.
- Reference meta_items when suggesting alternative itemization strategies in recommendations.
