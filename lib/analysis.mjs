 ...
 * how long the call took. The time spans the whole fallback chain, so a
 * provider outage shows up here rather than only in the logs.
 *
...
export function analysisFooter({ model, usage, ms }) {
  ...
  if (usage) {
    ...
    parts.push(
      `${usage.prompt_tokens.toLocaleString("en-US")} in / ${output.toLocaleString("en-US")} out tokens`,
    );
  }
