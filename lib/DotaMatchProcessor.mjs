...
function preparePlayers(match, focusPlayerId) {
  return match.players.map((player) => {
    ...
    const baseStats = {
      ...
      secondsDead: prepareSecondsDead(player),
    ...
    return {
      ...
      deathLog: prepareDeathLog(player),
...
function nameKiller(key, victimRaw) {
  if (key === victimRaw) return "themselves";
  if (key.startsWith("npc_dota_hero_")) return heroByRaw(key)?.name ?? key;
  if (key.includes("_tower")) return "a tower";
  if (key.startsWith("npc_dota_creep_") || key.includes("_siege")) {
    return "lane creeps";
  }
  if (key.startsWith("npc_dota_neutral_")) return "neutral creeps";
  if (key === "npc_dota_roshan") return "Roshan";
  return key;
}

// time_dead is absent for a death the match ended on, so the total covers completed
// respawns only. Buyback shortens it, which is the point: it is time off the map.
function prepareSecondsDead(player) {
  if (!player.deaths_log) return undefined;
  return player.deaths_log.reduce(
    (sum, death) => sum + (death.time_dead ?? 0),
    0,
  );
}

function prepareDeathLog(player) {
  const victimRaw = DotaConstants.heroes[player.hero_id]?.raw_name;
  return player.deaths_log?.map((death) => ({
    time: death.time,
    killedBy: nameKiller(death.key, victimRaw),
    ...(death.time_dead !== undefined ? { secondsDead: death.time_dead } : {}),
    ...(death.gold_lost > 0 ? { goldLost: death.gold_lost } : {}),
  }));
}
...
function prepareLog(match) {
  ...
  for (const player of Object.values(players)) {
    ...
    for (const message of player.kills_log) {
      const victim = heroByRaw(message.key);
      messages.push({
        time: message.time,
        message: `${hero.name} killed ${victim?.name}${message.smoke ? " from smoke" : ""}`,
      });
    }

    // Hero kills are already in the log from the killer's side
    for (const death of player.deaths_log ?? []) {
      if (
        death.key.startsWith("npc_dota_hero_") &&
        death.key !== hero.raw_name
      ) {
        continue;
      }
      messages.push({
        time: death.time,
        message: `${hero.name} was killed by ${nameKiller(death.key, hero.raw_name)}`,
      });
    }
 ...
 */
async function loadPopularItemsForGameMode(
  gameModeId,
  heroId,
...
- secondsDead: Total time each player spent dead, after buybacks. A death the match ended on is not counted
- deathLog: The focus player's deaths, each with its killer and secondsDead. goldLost appears only where the death cost gold, which a Turbo death never does
- secondsDead, deathLog and deaths to non-heroes in log are absent from matches parsed before OpenDota began emitting them
- log: A kill marked "from smoke" came under Smoke of Deceit or within 30 seconds of it breaking
...
DEATHS
- Read time spent dead against match duration and the lane opponent's, not the death count alone.
- Deaths to towers, creeps or neutrals, or repeated deaths to one hero, point to positioning; cite the times.
- Weigh a death by what followed it in log - a lost fight, tower or Roshan costs more than an isolated pickoff.
- Smoke kills separate planned ganks from chance pickoffs, for the player's own kills and for the deaths they suffered.
...
export async function generateAnalysisPrompt(
  ...
  if (getHeroItemPopularity) {
    const playerFromFull = fullMatch.players.find(
      (p) => p.account_id === playerId,
    );
    if (playerFromFull) {
