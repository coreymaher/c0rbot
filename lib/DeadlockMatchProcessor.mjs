/**
 * Pure business logic for processing Deadlock match data.
 * No AWS, Discord, OpenAI, or environment dependencies.
...
/**
 * Objective ID to human-readable name mapping
 *
 * Note: IDs 2, 6, 13 are from the old 4-lane system and are deprecated
 */
const OBJECTIVE_NAMES = {
  ...
  10: "Green Shrine",
  11: "Yellow Shrine",
  12: "Yellow Base Guardians",
  14: "Blue Base Guardians",
  15: "Green Base Guardians",
};

/**
 * Souls source ID to human-readable name mapping
 * Based on gold_sources data from match stats
 *
 * Note: Source 8 appears to correlate with comeback/rubber-banding mechanics (players who are
 * behind get bonus souls), but this is unconfirmed and the value doesn't appear in the client's
 * end game stats, so it's excluded from the output.
 */
const SOULS_SOURCE_NAMES = {
  1: "Enemy Kills",
  2: "Troopers",
  3: "Neutral Enemies",
  4: "Objectives",
  ...
  6: "Kill Assists",
  7: "Denies",
  // 8: Excluded - possibly comeback/rubber-banding bonus (unconfirmed, not shown in client)
...
/**
 * Get human-readable name for an objective
 * @param {number} objectiveId - The team_objective_id from the API
 * @returns {string} Human-readable objective name or "Unknown Objective"
 */
function getObjectiveName(objectiveId) {
  return (
    OBJECTIVE_NAMES[objectiveId] || `Unknown Objective (id ${objectiveId})`
  );
}

/**
 * Load Deadlock items data from API
 * @param {Object} options
 * @param {Object} options.cache - Optional cache implementation with get/set methods
 * @returns {Promise<{byId: Object, byClassName: Object}>}
 */
export async function loadItems({ cache = null, skipCache = false } = {}) {
  const cacheNamespace = "deadlock-ai-analyzer";
  const cacheKey = "items-v6"; // Increment version when item structure changes
...
// Property names mapped to values that should be excluded as default/meaningless
...
function buildItemEntry(itemData) {
  const itemEntry = {
  ...
  if (itemData.properties) {
    ...
    for (const [key, prop] of Object.entries(itemData.properties)) {
      ...
      const excludedValues = EXCLUDE_DEFAULTS[key];
      if (excludedValues && excludedValues.includes(String(value))) continue;
...
function preparePlayer(
  ...
  // Extract souls by source from final stats entry
  let soulsBySource = null;
  if (player.stats && player.stats.length > 0) {
    const lastStat = player.stats[player.stats.length - 1];
    if (lastStat.gold_sources) {
      soulsBySource = lastStat.gold_sources
        .map((source) => {
          // Exclude source 8 (unconfirmed, not shown in client)
          if (source.source === 8) return null;

          const totalSouls = (source.gold || 0) + (source.gold_orbs || 0);
          // Only include sources with non-zero souls
          if (totalSouls === 0) return null;

          return {
            source:
              SOULS_SOURCE_NAMES[source.source] ||
              `Unknown Source ${source.source}`,
            souls: totalSouls,
          };
        })
        .filter((entry) => entry !== null)
        .sort((a, b) => b.souls - a.souls); // Sort by souls descending
    }
  }
  ...
  // Extract final items for all players (items not sold at end of match)
  // Only store item class_names - LLM will look up details in all_items_reference
  const finalItems = [];
  for (const entry of player.items ?? []) {
    // Skip sold items (only keep items player has at end of match)
    if (entry.sold_time_s) continue;

    const itemData = itemsById[entry.item_id];
    if (!itemData) continue;

    // Skip abilities (only include actual items)
    if (itemData.hero || (itemData.heroes && itemData.heroes.length > 0))
      continue;

    finalItems.push(itemData.class_name || "unknown");
  }
  ...
  const baseStats = {
    ...
    ...(soulsBySource && { souls_by_source: soulsBySource }),
  };
  ...
  for (const entry of player.items ?? []) {
    ...
    if (itemData.hero || (itemData.heroes && itemData.heroes.length > 0)) {
      ...
      // Item - only store class_name and timing, LLM will look up details in all_items_reference
      const itemEntry = {
        game_time_s: entry.game_time_s,
        sold_at_s: entry.sold_time_s || null,
        item: itemData.class_name || "unknown",
      };

      if (entry.imbued_ability_id) {
        const imbuedAbility = itemsById[entry.imbued_ability_id];
        if (imbuedAbility) {
          itemEntry.imbued_ability = imbuedAbility.class_name || "unknown";
        }
      }
  ...
  // Build meta items context for focus player
  let metaItems = null;
  if (
    isFocus &&
    popularItemsData &&
    Array.isArray(popularItemsData) &&
    popularItemsData.length > 0
  ) {
    // popularItemsData is already sorted by win rate from DeadlockAPI
    // Filter to only tier 2+ items (item_tier >= 2)
    metaItems = popularItemsData
      .map((popularItem) => {
        const itemData = itemsById[popularItem.item_id];
        if (!itemData || !itemData.class_name) return null;

        // Only include tier 2+ items (skip tier 1)
        if (itemData.item_tier == null || itemData.item_tier < 2) return null;

        // win_rate is already calculated by DeadlockAPI as a percentage
        return {
          item: itemData.class_name,
          win_rate: Math.round(popularItem.win_rate),
        };
      })
    ...
    for (const obj of matchData.objectives) {
      if (obj.destroyed_time_s > 0) {
        events.push({
          ...
          owning_team: obj.team,
          objective_id: obj.team_objective_id,
          objective_name: getObjectiveName(obj.team_objective_id),
          creep_damage: obj.creep_damage || 0,
          player_damage: obj.player_damage || 0,
        });
      }
    }
  }
...
export function detectTeamfights(matchData) {
  ...
  for (let i = 0; i < allDeaths.length; i++) {
    ...
    if (fightDeaths.length >= MIN_DEATHS) {
      ...
      // A teamfight just requires MIN_DEATHS total deaths
      // One-sided teamfights (where one team dominates without losing anyone) are valid
...
export function generateCompactMatch(
  ...
  // Build deduplicated items reference:
  // - For focus player: ALL items they purchased (including sold items)
  // - For other players: Only final items (items they ended with)
  // Key by class_name (guaranteed unique) for LLM lookup
  const itemsMap = new Map();

  // First, add all items from focus player's purchase history
  const focusPlayer = matchData.players.find(
    (p) => p.account_id === focusPlayerId,
  );
  if (focusPlayer) {
    for (const entry of focusPlayer.items ?? []) {
      const itemData = itemsData.byId[entry.item_id];
      if (!itemData) continue;
      ...
      // Use class_name as key for deduplication (guaranteed unique)
      if (!itemsMap.has(itemData.class_name)) {
        itemsMap.set(itemData.class_name, buildItemEntry(itemData));
      }
    }
  }

  // Then add final items from all other players (will not overwrite focus player items)
  for (const player of matchData.players) {
    // Skip focus player (already processed above)
    if (player.account_id === focusPlayerId) continue;
    ...
    for (const entry of player.items ?? []) {
      ...
      // Use class_name as key for deduplication (won't overwrite if already exists from focus player)
      if (!itemsMap.has(itemData.class_name)) {
        itemsMap.set(itemData.class_name, buildItemEntry(itemData));
      }
    }
  ...
  // Add items from popular items data to ensure LLM has full details for meta context
  if (popularItemsData && Array.isArray(popularItemsData)) {
    for (const popularItem of popularItemsData) {
      const itemData = itemsData.byId[popularItem.item_id];
      if (!itemData) continue;
      ...
      // Add to reference if not already present
      if (!itemsMap.has(itemData.class_name)) {
        itemsMap.set(itemData.class_name, buildItemEntry(itemData));
      }
    }
  ...
  // Convert itemsMap to a plain object for LLM to easily look up by class_name
  const all_items_reference = {};
  for (const [class_name, itemData] of itemsMap.entries()) {
    all_items_reference[class_name] = itemData;
  }
  ...
  return {
    ...
    all_items_reference: all_items_reference,
    players: players,
  };
}
...
- CRITICAL: Always use the winning_team value to determine match outcome
- Do NOT infer the winner from timeline events or objective counts
...
SOULS ECONOMY
- Souls are the primary in-game currency used to purchase items and abilities.
- Players earn souls from multiple sources throughout the match, tracked in souls_by_source (sorted by amount, descending).
- Efficient soul farming is critical for maintaining item and level advantages.
- Compare souls_by_source across players to identify farming efficiency.
- Low trooper souls may indicate poor laning/last-hitting; low neutral souls may indicate lack of jungle control or prioritization.
- High objective souls indicate strong participation in taking enemy structures.
...
HERO PICK & TEAM COMPOSITION
- Analyze how the focus player's hero fits their team's overall composition and strategy.
- Consider if the hero complements allied heroes (damage synergies, initiation combos, protection for fragile carries).
- Assess if the hero addresses team composition gaps (mobility, crowd control, healing, wave clear, late-game scaling).
- Evaluate if the hero counters enemy threats (tankiness vs burst damage, mobility vs immobile heroes, sustain vs poke).
- Reference all_items_reference and final_items for all players to understand team strategies without assuming item knowledge.
- Identify if the team composition or hero matchup created inherent advantages or disadvantages.
- Focus on significant composition factors; don't critique every pick unless notably impactful.
...
- Player item references use class_name identifiers (e.g., "upgrade_magic_burst"). Look up these identifiers in all_items_reference to get full details.
- all_items_reference is a map where keys are class_name identifiers and values contain: name (display name), description, slot_type, is_active, cost, tier, and properties.
- Items have tiers 1-4, where higher tier items are more expensive and generally more impactful.
- Example: If a player has item "upgrade_magic_burst" in their final_items, look up all_items_reference["upgrade_magic_burst"] to see it's "Extra Charge" with full stats.
- You may reference items by name when they appear in all_items_reference (items from this specific match).
- CRITICAL: Do NOT reference items from your training data or suggest specific item names in recommendations - items change frequently and your knowledge may be outdated. In recommendations, describe item properties/effects instead (e.g., "consider items that provide healing reduction" not "buy Decay").
- Analyze itemization strategy by examining item descriptions, properties, slot types (weapon/vitality/spirit), tiers, and costs from all_items_reference.
- Evaluate if item builds address the focus player's hero weaknesses (survivability, mobility, sustain, damage output).
- Consider if items counter specific enemy threats based on their properties (damage mitigation vs high damage dealers, healing reduction, mobility/escape tools).
- Assess if items fill team composition gaps (team lacks initiation, healing, wave clear, or other utility).
- Compare focus player's itemization approach against their role counterpart on the enemy team.
- Reference damage_received and damage_dealt to evaluate if defensive/offensive itemization matched actual threats and opportunities.
- Analyze item diversity across slot types (balanced weapon/vitality/spirit vs heavily specialized builds).

META ITEMS CONTEXT
- The focus player may have a meta_items field showing high-performing items for their hero at this skill level.
- Each meta item includes: item (class_name identifier) and win_rate (%).
- Items are ordered by win_rate (descending) - highest win rate items appear first.
- All items have been filtered to include only those with sufficient sample size (100+ matches minimum).
- CRITICAL: The item field is a class_name identifier (e.g., "upgrade_target_stun"). You MUST look it up in all_items_reference to get the actual display name (e.g., "Knockdown"). NEVER invent item names based on class_name patterns.
- Use meta_items to evaluate if the player's itemization aligned with proven high-win-rate builds for their hero.
- Items at the top of the list are the strongest performing items for this hero at this rank.
- Consider whether the player built high-win-rate items, or if they missed opportunities.
- Deviations from high-win-rate items may be situationally appropriate (countering specific enemies, filling team gaps, adapting to match flow).
- Meta builds are guidelines, not rules - adaptive itemization for match context is valid and often necessary.
- When referencing meta items in recommendations, always use the display name from all_items_reference, never the class_name.
...
- Consider alternative itemization strategies that could address weaknesses or improve effectiveness (describe item properties/effects, not names).
- Suggest hero picks or team composition considerations for future matches when relevant.
- Include at least one recommendation that builds on demonstrated strengths.
...
- Mention notable hero pick decisions when they significantly impacted the match (strong counters, composition synergies, or problematic matchups).
- Reference itemization approach when it was particularly effective or could have been improved.
